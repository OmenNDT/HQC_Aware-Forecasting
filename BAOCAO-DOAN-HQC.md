...
Đồ án dùng đường cong học để chọn **kiến trúc** (tuyến tính so với mạng nơ-ron), không bằng
quy tắc kinh nghiệm và không bê nguyên kiến trúc của công trình tham chiếu — vì lượng dữ liệu
khác nhau.

Cần phân biệt rõ hai câu hỏi khác nhau, vì chúng cần hai loại bằng chứng khác nhau:

| Câu hỏi | Bằng chứng dùng |
|---|---|
| **Kiến trúc nào?** tuyến tính hay phi tuyến | đường cong học (mục 4.4) |
| **Giữ bao nhiêu chiều?** | tiêu chí tách (mục 2.3.5) |

Đường cong học **không** trả lời được câu thứ hai trên dữ liệu này — mục 2.3.5 giải thích vì sao.

### 2.3.5. Chọn số chiều giữ lại: tiêu chí tách thay tiêu chí phương sai

Với mô hình dùng để **nén dữ liệu**, số chiều chọn theo lượng phương sai giữ được. Với mô hình
dùng để **phát hiện bất thường**, tiêu chí đó không còn đúng — và hai tiêu chí cho hai đáp số
khác nhau.

Lý do: giữ nhiều chiều làm mô hình tái tạo tốt hơn, nhưng đến một mức nào đó nó bắt đầu dựng
lại được **cả** kiểu quan hệ bất thường. Khi đó sai số ở nhóm bất thường tụt xuống ngang nhóm
bình thường và tín hiệu phát hiện biến mất. Trường hợp giới hạn: giữ hết mọi chiều thì mô hình
chỉ sao chép đầu vào sang đầu ra, sai số bằng không ở mọi nơi, và mô hình vô dụng.

Do đó đồ án chọn số chiều theo **tỉ số tách** — sai số trung bình nhóm bất thường chia sai số
trung bình nhóm bình thường. Đây là đại lượng đo trực tiếp thứ mô hình cần làm được, khác với
phương sai giải thích (đo thứ mô hình giữ lại được).
...
| PCN-2 | Dung lượng mô hình cân xứng dữ liệu | kiến trúc xác minh bằng đường cong học; số chiều bằng tiêu chí tách |
...
3. **Chứng minh mô hình đúng kích thước triển khai được ở mọi nơi.** Mô hình được chọn nhỏ
   vì bằng chứng thực nghiệm (đường cong học chọn kiến trúc, tiêu chí tách chọn số chiều),
   không phải vì mục đích dễ nhúng; nhưng chính vì nhỏ mà
...
## 4.4. Chọn kiến trúc và số chiều giữ lại

### 4.4.1. Chọn kiến trúc bằng đường cong học

Chia dữ liệu **theo khối thời gian** (không xáo trộn): huấn luyện trên phần đầu, giữ 25% giờ
khỏe cuối làm kiểm định. Đã kiểm cửa sổ kiểm định vẫn thuần khỏe (mọi kênh lệch dưới 3,5% so
phần huấn luyện) trước khi đọc kết quả.

**Bảng 4.4: Đường cong học — so sánh kiến trúc**

| Kiến trúc | Số trọng số | Sai số kiểm định | Chênh kiểm định − huấn luyện |
|---|---|---|---|
| **Tuyến tính (6 chiều)** | ~144 | **0,542** | **+0,223** |
...
Kết luận "mô hình tuyến tính đủ tốt, mạng nơ-ron sâu bị quá khớp" là một **kết quả có giá trị**
với khối lượng dữ liệu hiện có, không phải hạn chế của đồ án.

### 4.4.2. Chọn số chiều: bốn tiêu chí phương sai không đồng thuận

![Ba tiêu chí chọn số chiều](/home/sontn/.claude-science/orgs/49e3c671-cbbe-4f16-8bad-8cba412d2a77/artifacts/proj_0c140d6162d4/5133d805-7ddd-4aee-8620-1d214cb40dfe/vce0a4e0f_hqc_chon_so_chieu.png)

**Hình 4.6:** (a) Trị riêng của 24 chiều, kèm vị trí bốn tiêu chí phương sai. (b) Sai số kiểm
định và chênh tương đối theo số chiều. (c) Tỉ số tách theo số chiều, đỉnh tại k = 6.

**Bảng 4.5: Bốn tiêu chí có tên cho bốn đáp số khác nhau**

| Tiêu chí | Cách xác định | Kết quả |
|---|---|---|
| Kaiser | giữ các chiều có trị riêng > 1 | k = 8 |
| Điểm gãy đồ thị scree | chỗ trị riêng tụt mạnh nhất | k = 1 (không dùng được) |
| Giữ ≥80% phương sai | | k = 8 |
| Giữ ≥90% phương sai | | k = 11 |
| Giữ ≥95% phương sai | | k = 13 |

Trị riêng của bộ dữ liệu này giảm rất **thoải** (19,8% → 14,5% → 11,9% → 11,4% → 7,4% → 6,0%),
không có bậc thang rõ (hình 4.6a) — nên tiêu chí điểm gãy vô dụng và ba tiêu chí còn lại cho ba
con số khác nhau. **Không có "con số đúng duy nhất" theo tiêu chí phương sai.**

### 4.4.3. Đường cong học không đặt được trần cho số chiều

**Bảng 4.6: Sai số theo số chiều giữ lại**

| Số chiều | Sai số huấn luyện | Sai số kiểm định | Chênh tuyệt đối | Chênh tương đối |
|---|---|---|---|---|
| 2 | 0,579 | 0,727 | +0,148 | 25,5% |
| 4 | 0,448 | 0,571 | +0,122 | 27,3% |
| **6** | 0,369 | 0,463 | +0,093 | **25,2%** |
| 10 | 0,214 | 0,285 | +0,072 | 33,5% |
| 16 | 0,038 | **0,058** | +0,020 | **52,4%** |

Sai số kiểm định **giảm đều tới k = 16**, không có điểm gãy (hình 4.6b). Nguyên nhân: mô hình
quá gọn nên ngay k = 16 vẫn còn 1,2 mẫu trên mỗi tham số — chưa vào vùng quá khớp rõ rệt theo
chỉ số này.

**Một cái bẫy phải nêu:** chênh **tuyệt đối** nhỏ dần ở k lớn là hiệu ứng máy móc — cả hai sai
số cùng tiến về không nên hiệu của chúng cũng nhỏ. Chênh **tương đối** cho thấy sự thật ngược
lại: 25,2% ở k = 6 tăng lên **52,4%** ở k = 16, tức quá khớp **nặng dần**.

### 4.4.4. Tiêu chí tách — căn cứ thật để chọn số chiều

**Bảng 4.7: Tỉ số tách theo số chiều giữ lại**

| Số chiều | 2 | 4 | 5 | **6** | 7 | 8 | 10 | 13 | 16 | 24 |
|---|---|---|---|---|---|---|---|---|---|---|
| Tỉ số tách | 1,53× | 1,76× | 1,84× | **1,94×** | 1,90× | 1,89× | 1,29× | 1,20× | 1,12× | — |

Có **đỉnh rõ tại k = 6**, rồi sụp nhanh (hình 4.6c). Trường hợp giới hạn k = 24 (giữ hết mọi
chiều) cho sai số tái tạo **1,07 × 10⁻¹⁵** trên giai đoạn nền và **cùng mức 10⁻¹⁵** trên cả hai
nhóm của tập đánh giá — tức mô hình tái tạo hoàn hảo cả dữ liệu bất thường, **mất hoàn toàn**
khả năng phát hiện. Đây là minh chứng trực tiếp cho tiêu chí hai vế ở mục 2.3.5.

### 4.4.5. Kiểm độ nhạy

Báo động giả trên giai đoạn nền ở các lựa chọn quanh k = 6:

| Số chiều | 4 | 5 | **6** | 7 | 8 |
|---|---|---|---|---|---|
| Báo động giả | 1,08% | 1,21% | **1,08%** | 0,94% | 0,94% |
| Tỉ số tách | 1,76× | 1,84× | **1,94×** | 1,90× | 1,89× |

Toàn bộ khoảng k = 5–8 cho báo động giả 0,94–1,21% và tỉ số tách 1,84–1,94×. **Kết luận không
nhạy** với lựa chọn cụ thể trong khoảng này — điều quan trọng, vì nếu chỉ k = 6 hoạt động mà
k = 5 hay 7 thì sập thì kết quả là ngẫu nhiên.

### 4.4.6. Quy trình chọn số chiều, gói lại

| Bước | Việc | Kết quả trên dữ liệu này |
|---|---|---|
| 1 | Chặn trên bằng ngân sách dữ liệu (số tham số so số mẫu) | k < 20 |
| 2 | Thu hẹp bằng tiêu chí phương sai có tên | k ∈ [8, 13] |
| 3 | Kiểm quá khớp bằng đường cong học, dùng chênh **tương đối** | **không kết luận được** |
| 4 | Quyết bằng tiêu chí tách | **k = 6** |
| 5 | Kiểm độ nhạy quanh lựa chọn | ổn định trong k = 5–8 |

Bước 3 không kết luận được là một **kết quả cần nêu thẳng**, không phải chỗ để bỏ qua.
...
| **3**<br>17–23/08 | Huấn luyện & Tối ưu mô hình<br>*(bước 3–4)* | Xây và huấn luyện các kiến trúc tái tạo; chạy đường cong học chọn kiến trúc và tiêu chí tách chọn số chiều; tính giới hạn kiểm soát thống kê 99%; xây điểm sức khỏe 0–100 và lớp đóng góp từng đầu đo; thực nghiệm nhiều phương án lượng tử hóa và chốt phương án. | Nghĩa | Mô hình đã huấn luyện kèm giới hạn kiểm soát; báo cáo chọn mô hình có căn cứ; bảng so sánh các phương án lượng tử hóa; mô hình đã nén. |
...
3. Chạy **đường cong học** để chọn **kiến trúc** (tuyến tính so với mạng nơ-ron) bằng bằng chứng
   thực nghiệm thay vì quy tắc kinh nghiệm; kiểm cửa sổ kiểm định vẫn thuần khỏe trước khi đọc
   kết quả. Riêng **số chiều giữ lại** chọn bằng **tiêu chí tách**, không bằng đường cong học —
   xem mục 2.3.5.
